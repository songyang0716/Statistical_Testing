...
# Without correcting for covariance of pre-post data points of each sample
sig_sim <- 0 
sim <- 10000
...
# No effects in post period
# Correct for covariance of pre-post data points of each sample
sig_sim <- 0 
for (i in 1:sim){
  pre <- rpois(samples_per_cohort, 10)
  post <- pre + rnorm(samples_per_cohort, mean=0, sd=1)
...
# Linear regression (Linear regression assumes each samples are independent to each other, so I think the result should be similar to the one without correcting for the pre-post covariance)
sig_sim <- 0 
for (i in 1:sim){
  pre <- rpois(samples_per_cohort, 10)
  post <- pre + rnorm(samples_per_cohort, mean=0, sd=1)
